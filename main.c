    ...
    do {
        ...
        switch (choix) {
            case 1: {
                ...
                int c; // Variable pour nettoyer le buffer
                ...
                // Nettoyage du buffer clavier avant d'entrer dans la boucle
                while ((c = getchar()) != '\n' && c != EOF);

                // --- DEBUT : Ajout du niveau de Licence sécurisé ---
                int saisie_valide = 0;
                do {
                    ...
                    if (scanf("%d", &nouvel_etudiant.niveau_licence) != 1) {
                        ...
                        // On vide la mauvaise saisie (ex: si l'utilisateur a tapé "abc")
                        while ((c = getchar()) != '\n' && c != EOF); 
                    } else {
                        // Saisie numérique réussie. On nettoie au cas où l'utilisateur a tapé "1abc"
                        while ((c = getchar()) != '\n' && c != EOF);
                        
                        // On vérifie que c'est bien L1, L2 ou L3
                        if (nouvel_etudiant.niveau_licence >= 1 && nouvel_etudiant.niveau_licence <= 3) {
                            saisie_valide = 1; // Condition remplie, on autorise la sortie de la boucle
                        } else {
                            printf("[ERREUR] Le niveau doit etre 1, 2 ou 3.\n");
                        }
                    }
                } while (!saisie_valide);
                // --- FIN : Ajout du niveau de Licence ---
                ...
                if (ajouter_etudiant_statique(ma_base_statique, nouvel_etudiant)) {
                    ...
                    printf("\n[SUCCES] Etudiant enregistre en L%d !\n", nouvel_etudiant.niveau_licence);
                }
                break;
            }
